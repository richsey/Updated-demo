            ...
            {profile && (
              <div className="relative" ref={menuRef}>
                ...
                >
                  ...
                  <span className="hidden sm:inline text-foreground/80">{profile.full_name}</span>
                  <span className="hidden sm:inline text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-medium">Lecturer</span>
                  <ChevronDown className={`h-3 w-3 transition-transform ${menuOpen ? "rotate-180" : ""}`} />
                </button>

                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-44 rounded-xl border border-border/60 bg-white shadow-lg z-50 overflow-hidden animate-in fade-in-0 slide-in-from-top-2 duration-150">
                    <Link
                      to="/lecturer/profile"
